...
__global__ void flash_attn_kernel_fp16_v7(
...
#pragma unroll
    for (int vec_offset = 0; vec_offset < Br * HEAD_SIZE; vec_offset += NUM_THREADS * VEC_SIZE) {
        ...
        const uint4* query_fp4_ptr = reinterpret_cast<const uint4*>(&query[q_b_offset + q_h_offset + q_blk_offset + vec_idx]);
        uint4* Q_fp4_ptr = reinterpret_cast<uint4*>(&Q[vec_idx]);
...
#pragma unroll
        for (int vec_offset = 0; vec_offset < Bc * HEAD_SIZE; vec_offset += NUM_THREADS * VEC_SIZE) {
            const int vec_idx = vec_offset + tid * VEC_SIZE;
            const uint4* key_fp4_ptr = reinterpret_cast<const uint4*>(&key[kv_b_offset + kv_h_offset + kv_blk_offset + vec_idx]);
            uint4* Kj_fp4_ptr = reinterpret_cast<uint4*>(&Kj[vec_idx]);
            *Kj_fp4_ptr = *key_fp4_ptr;
            const uint4* value_fp4_ptr = reinterpret_cast<const uint4*>(&value[kv_b_offset + kv_h_offset + kv_blk_offset + vec_idx]);
            uint4* Vj_fp4_ptr = reinterpret_cast<uint4*>(&Vj[vec_idx]);
...
#pragma unroll
            for (int offset = WARP_SIZE / 2; offset > 0; offset /= 2) {
                row_max_tile = fmaxf(row_max_tile, __shfl_xor_sync(0xFFFFFFFF, row_max_tile, offset));
            }
            ...
            s_ij = __expf(s_ij - row_max_new_i);
...
#pragma unroll
            ...
            row_exp_sum = (thread_group_id == i) ? row_exp_sum * expf(row_max_prev - row_max_new) + s_ij : row_exp_sum;
        }
        __syncthreads();
...
#pragma unroll
            for (int d = 0; d < HEAD_SIZE; d += VEC_SIZE * THREAD_GROUP_SIZE) {
                uint4* out_fp4_ptr = reinterpret_cast<uint4*>(&out[q_b_offset + q_h_offset + q_blk_offset + thread_group_id * HEAD_SIZE + d + VEC_SIZE * thread_group_offset]);
                __half out_tmp_fp16[VEC_SIZE];
#pragma unroll
                for (int dd = 0; dd < VEC_SIZE; dd++) {
                    out_tmp_fp16[dd] = __float2half((1 / row_exp_sum) * out_tmp[d / THREAD_GROUP_SIZE + dd]);
                }
                const uint4* out_tmp_fp4_ptr = reinterpret_cast<const uint4*>(&out_tmp_fp16);
