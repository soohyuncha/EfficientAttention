...
#pragma unroll
    for (int vec_offset = 0; vec_offset < Br * HEAD_SIZE; vec_offset += NUM_THREADS * 8) {
        const int vec_idx = vec_offset + tid * 8;
...
#pragma unroll
        for (int vec_offset = 0; vec_offset < Bc * HEAD_SIZE; vec_offset += NUM_THREADS * 8) {
            const int vec_idx = vec_offset + tid * 8;
...
#pragma unroll
        ...
        if ((lane_id < WMMA_M) && (q_blk_idx * Br + (warp_id * WMMA_M + lane_id) < q_len)) {
            ...
            // Write scaled output into global memory only at the last j-th tile
            if (j == Tc - 1) {
#pragma unroll
