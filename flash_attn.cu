...
//                if ((tid < Br) && (q_blk_idx * Br + tid) < q_len) {
                if ((lane_id < WMMA_M) && (q_blk_idx * Br + (warp_id * WMMA_M + lane_id) < q_len)) {
                    int row_idx = warp_id * WMMA_M + lane_id;
//                    out_prev = __half2float(out[q_b_offset + q_h_offset + q_blk_offset + row_idx * HEAD_SIZE + (d + dd)]);
                    out_tmp[d + dd] = out_tmp[d + dd] * __expf(row_max_prev - row_max_new) + O[row_idx * WMMA_N + dd];
//                    out[q_b_offset + q_h_offset + q_blk_offset + row_idx * HEAD_SIZE + (d + dd)] = __float2half(out_new);
                }
            }
            __syncthreads();
        }
        ...
        if ((lane_id < WMMA_M) && (q_blk_idx * Br + (warp_id * WMMA_M + lane_id) < q_len)) {
            ...
            if (j == Tc - 1) {
                for (int d = 0; d < HEAD_SIZE; d++) {
                    //out[q_b_offset + q_h_offset + q_blk_offset + row_idx * HEAD_SIZE + d] = __float2half((1 / row_exp_sum_new) * __half2float(out[q_b_offset + q_h_offset + q_blk_offset + row_idx * HEAD_SIZE + d])); 
